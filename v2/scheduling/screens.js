(() => {
  ...
  M.screens.schedulesList = (ctx) => {
    const state=stateView(ctx,'horarios'); if(state) return `${C().pageHeader('Horarios','',ctx.readonly?null:{label:'Nuevo horario',href:'/horarios/nuevo',icon:'plus'})}${state}`;
    ...
    return `${C().pageHeader('Horarios','',ctx.readonly?null:{label:'Nuevo horario',href:'/horarios/nuevo',icon:'plus'})}
  ...
  M.screens.scheduleEditor = (ctx) => {
    ...
    const isNew = ctx.path === '/horarios/nuevo';
    let html=`${C().pageHeader(isNew?'Nuevo horario':`Ficha ${s.ficha} — Período ${s.period}`,'',null,{label:'Volver a Horarios',href:'/horarios'})}
      ...
      <div class="panel-body"><div class="schedule-editor-header"><div class="form-field"><label>Ficha</label><select><option value="" disabled ${isNew ? 'selected' : ''}>Seleccione una ficha</option><option ${!isNew ? 'selected' : ''}>2874412 — ADSO</option><option>3011550 — Contabilidad</option></select></div><div class="form-field"><label>Período</label><input ${isNew ? 'placeholder="Ej: 2026-2"' : 'value="2026-2"'}></div><div class="form-field"><label>Nombre</label><input ${isNew ? 'placeholder="Ej: ADSO — Trimestre III"' : 'value="ADSO — Trimestre III"'}></div></div></div></section>
  ...
  M.screens.learnerSchedule = (ctx) => {
    ...
    const calendarSessions=[{id:'ses-01',dayIndex:0,slotIndex:0,title:'Desarrollo de software',time:'07:00–10:00',environment:'Lab. A-204 · Juan Pérez'},{id:'ses-02',dayIndex:1,slotIndex:1,title:'Modelado de BD',time:'10:00–13:00',environment:'Aula B-105 · Carolina Rojas'},{id:'ses-03',dayIndex:2,slotIndex:2,title:'Servicios web',time:'13:00–16:00',environment:'Lab. A-204 · Juan Pérez'}];
    return `${C().pageHeader('Mi horario','Ficha 2874412 · Semana del 10 al 14 de agosto de 2026')}<div class="alert alert-info" style="margin-bottom:16px">${icon('lock')}<div>Ves únicamente <strong>tu ficha (2874412)</strong> — acceso propio <code>SCH_VIEW_OWN</code>. No puedes consultar horarios de otras fichas.</div></div><div class="page-actions" style="justify-content:space-between;margin-bottom:20px"><button class="btn btn-secondary">‹ Semana anterior</button><button class="btn btn-secondary">Semana siguiente ›</button></div><div class="data-table-wrap">${C().weekCalendar(calendarSessions)}</div>`;
  };
