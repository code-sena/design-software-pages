(() => {
  ...
  M.screens.auditList=(ctx)=>{
    ...
    let html=`${C().pageHeader('Auditoría','Registro de solo lectura de las acciones del sistema.')}${C().filters([{label:'Actor (UUID)',type:'text',placeholder:'UUID'},{label:'Tipo de entidad',type:'text',placeholder:'schedule'},{label:'Tipo de evento',type:'text',placeholder:'SCHEDULE_PUBLISHED'},{label:'Servicio origen',options:['Todos','scheduling-service','iam-service','document-service']},{label:'Desde',type:'date'}],'Exportar')}${C().table([{label:'Recibido',key:'date'},{label:'Ocurrido en origen',key:'date'},{label:'Evento',key:'event'},{label:'Servicio origen',render:r=>C().status('INFO',r.service)},{label:'Actor',key:'actor'},{label:'Entidad',render:r=>`${r.entity} · ${r.entityId}`},{label:'Acción',render:()=>'<button class="btn btn-secondary btn-sm" data-open-modal="audit">Ver payload</button>'}],D().audit,{caption:'Eventos de auditoría',total:1200,start:1,end:4,pageSize:10,cursor:true})}<p class="panel-subtitle" style="margin:12px 0">Registro append-only · retención mínima 7 años · particionado mensual · datos de más de 2 años se archivan en frío.</p>`;
    if(ctx.query.get('modal')==='audit') html+=auditModal();
    return html;
  };
  function auditModal(){ const a=D().audit[0];return C().modal('Detalle de registro de auditoría',`<div style="display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap"><div><strong>${a.event}</strong><div style="margin-top:8px">${C().status('INFO',a.service)}</div></div><div>${C().details([['Recibido',a.date],['Ocurrido en origen',a.date]])}</div></div><h3>Identificación</h3>${C().details([['ID de evento',`${a.id} ${icon('copy')}`],['Actor',a.actor],['Entidad',`${a.entity} · ${a.entityId}`]])}<h3>Payload</h3><pre class="code-view">{
  "event_id": "evt-01",
