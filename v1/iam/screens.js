(() => {
  ...
  M.screens=M.screens||{};
  M.screens.login=(ctx)=>`<main class="auth-page"><div class="auth-shell">${authBrand()}<section class="card auth-card"><h1>Ingresar</h1><p>Accede con tu cuenta institucional.</p>${ctx.state==='error'?`<div class="alert alert-critical" style="margin-bottom:20px">${icon('warning')}Correo o contraseña incorrectos.</div>`:''}<form data-login-form><div class="form-field"><label for="login-email">Correo</label><input id="login-email" type="email" value="coordinador@sena.edu.co" autocomplete="username"></div><div class="form-field" style="margin-top:16px"><label for="login-password">Contraseña</label><div class="password-wrap"><input id="login-password" type="password" value="Demo2026!" autocomplete="current-password"><button class="icon-btn" type="button" data-password-toggle aria-controls="login-password" aria-label="Mostrar contraseña">${icon('eye')}</button></div></div><div style="display:flex;justify-content:flex-end;margin:8px 0"><a class="btn btn-link" href="#/forgot-password">¿Olvidó su contraseña?</a></div><button class="btn btn-primary" style="width:100%" type="submit">Ingresar</button></form><p class="auth-note">Uso exclusivo de personal y comunidad académica autorizada.</p></section></div></main>`;
