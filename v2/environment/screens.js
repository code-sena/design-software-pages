(() => {
  ...
  M.screens.environmentDetail=(ctx)=>{
    ...
    return `${C().pageHeader(env.name,`${env.type} · ${env.capacity} personas · ${env.location}`,null,{label:'Volver a Disponibilidad',href:'/disponibilidad'})}
      ...
      <section class="panel"><header class="panel-header"><div><h2 class="panel-title">Aptitud del ambiente</h2><p class="panel-subtitle">Aforo, inspección y certificación que condicionan la asignación de sesiones.</p></div></header><div class="panel-body">${C().details([['Aforo',`${env.capacity} personas`],['Estado de inspección','Vigente · última 20/07/2026'],['Certificación requerida','Ninguna'],['Mantenimiento programado','Jueves 13/08 · 13:00–16:00']])}</div></section>
      <section class="panel"><header class="panel-header"><div><h2 class="panel-title">Ocupación por franja</h2><p class="panel-subtitle">Lunes a viernes, semana seleccionada.</p></div></header><div class="panel-body data-table-wrap"><div class="occupancy-grid"><div></div>${['Lunes','Martes','Miércoles','Jueves','Viernes'].map(d=>`<div><strong>${d}</strong></div>`).join('')}${['07:00–10:00','10:00–13:00','13:00–16:00'].map((slot,ri)=>`<div><strong>${slot}</strong></div>${cells.slice(ri*5,ri*5+5).map(v=>`<div title="${v==='Ocupada'?'Reservado por ficha 123456':v==='Mantenimiento'?'En reparación':''}">${C().status(v==='Libre'?'AVAILABLE':v==='Ocupada'?'CONFLICT':'ARCHIVED',v)}${v!=='Libre'?`<br><small style="color:var(--color-text-muted);font-size:11px">${v==='Ocupada'?'Ficha 123456':'Reparación'}</small>`:''}</div>`).join('')}`).join('')}</div></div><footer class="panel-footer"><a href="#/disponibilidad/ambientes/${env.id}?report=utilization">Ver reporte de utilización (horas)</a></footer></section>`;
  };
